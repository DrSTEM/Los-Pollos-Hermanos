...
// === BASIC AUTH SETUP (uses config.js) ===
if (config?.challenge !== false) {
  console.log(chalk.green("🔒 Password protection is enabled! Listing logins below"));
  // Print configured users to console (mirrors your original snippet)
  ...
  await app.register(fastifyBasicAuth, {
    // validate function called for each request that uses basic auth
    validate: async (username, password, req, reply) => {
      if (!config.users || !(username in config.users) || config.users[username] !== password) {
        // throwing makes the plugin return 401
        throw new Error("Invalid credentials");
      }
    },
    // If authenticate is true the plugin decorates the reply/request with `basicAuth` function.
    authenticate: true
  });

  // Add a global onRequest hook to enforce basic auth for all incoming requests.
  // If you want to protect only certain routes, move this hook or apply per-route preHandlers.
  app.addHook("onRequest", async (request, reply) => {
    try {
      // app.basicAuth is provided by the plugin
      await app.basicAuth(request, reply);
    } catch (err) {
      // Ensure the browser shows a basic auth prompt
      reply.header("WWW-Authenticate", 'Basic realm="Secure Area"');
      return reply.code(401).send({ error: "Unauthorized" });
    }
  });
} else {
  console.log(chalk.yellow("🔓 Password protection is disabled (config.challenge === false)"));
}
// ==========================================
